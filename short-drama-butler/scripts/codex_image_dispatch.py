...
from keyframe_consistency import is_obsolete_phase_split_plan
from keyframe_prompt import build_frame_brief, load_parsed_storyboard
...
def _refused_keyframe(episode_id: str, shot_id: str, frame_kind: str, reason: str) -> dict[str, Any]:
    return {
        "allowed": False,
        "kind": "keyframe",
...
def inspect_image_generation_context(project_root: Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "rules": [
            ...
            "Keyframe stills are generated by one sub-agent per frame, including refinements and redos; the main agent reviews only and must not generate or view_image keyframe masters itself.",
            ...
            "Default: one generate with all masters in view_image_paths when they fit in 5. Split stages only for the 5-image cap or atomic relationship groups; a background-only stage is overflow, not the normal 3-asset path.",
            "Do not invent a background-only stage to inject night lighting, and do not cancel a dispatch to rewrite the prompt.",
            "If the shot is night/dusk/dawn and the scene has no matching confirmed view, stop and dispatch-asset a time-matched scene plate first.",
            "QA locks identity, space, and time of day; intended blocking or pose changes from the storyboard must pass.",
            "Do not chain shot2→shot3→shot4 identity from previous generated frames.",
            "Print brief.text verbatim before viewing references or generating.",
...
def dispatch_keyframe(
    ...
    obsolete = is_obsolete_phase_split_plan(current_plan)
    existing = None if obsolete else current_keyframe_dispatch(
        root, episode_id, shot_id, frame_kind, stage_id
    )
    if existing:
        dispatch = existing
    else:
        next_stage = None if obsolete else _next_planned_stage(current_plan, stage_id)
        if current_plan and next_stage:
            ...
            try:
                ...
                message = str(error)
                if "没有已确认的" in message and "视图" in message:
                    return _refused_keyframe(episode_id, str(shot_id), str(frame_kind), message)
                raise ImageCanonError(message) from error
    ...
    mismatch = keyframe_time_gate_reason(
        root, episode_id, str(shot_id), str(frame_kind), dispatch.get("input_images")
    )
    if mismatch:
        return _refused_keyframe(episode_id, str(shot_id), str(frame_kind), mismatch)
    plan_for_mode = current_keyframe_plan(root, episode_id, shot_id, frame_kind) or {}
    generation_mode = plan_for_mode.get("generation_mode") or "staged_edit"
    brief = _keyframe_brief(root, episode_id, str(shot_id), str(frame_kind), dispatch.get("input_images"))
    return {
        "allowed": True,
        "kind": "keyframe",
        "episode_id": episode_id,
        ...
        "codex_instructions": [
            "出图前必须先原样输出 brief.text（1. 本图故事 2. 本镜引用素材 3. 制作时必须注意），再读参考图，再 generate。",
            ...
            "generation_mode 为 single_pass 时只出这一次图，不要再拆先背景后人物，也不要在出图中改脚本或取消重写提示词。",
            "禁止自制构图叠加层，禁止用上一镜产图替换场景母版。",
            "时间光线跟场景母版和本 prompt，不要为了补深夜另开背景阶段。",
            "本镜若是深夜/黄昏/黎明，场景必须有对应已确认视图；没有就先 dispatch-asset 补时段母版，禁止用白天图出夜戏。",
            "record-qa 只锁身份、空间、时段；分镜要求的走位和姿势变化必须通过，不得用上一帧站位否决本帧动作。",
...
def _keyframe_brief(
    ...
    """Rebuild the pre-generation memory card from the storyboard and this dispatch."""
    episode_dir = _episode_dir(project_root, episode_id)
    parsed = load_parsed_storyboard(episode_dir / "storyboard.md")
    shot: dict[str, Any] = {"shot_id": shot_id}
    manifest_path = episode_dir / "keyframe-execution-manifest.json"
    if manifest_path.is_file():
        payload = json.loads(manifest_path.read_text(encoding="utf-8"))
        wanted = str(shot_id).zfill(2)
        for item in payload.get("shots") or []:
            if str(item.get("shot_id") or "").zfill(2) == wanted:
                shot = item
                break
    return build_frame_brief(parsed, shot, frame_kind, input_images=input_images)
