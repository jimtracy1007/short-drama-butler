...
**新开一个 Codex 窗口做图，会不会漂？** 不会按纯文字重新设计：先启用 `$short-drama-butler`，运行 `short-drama-butler/scripts/butler.py inspect` 和 `status`，再用 `dispatch-keyframe` / `dispatch-asset` 把返回的已有素材图读进当前对话；没有这些参考图就不能生成。建集、生产单、登记素材、确认剧本和记录出图结果也都走同一条 `butler.py`，不要自己拼 Python。`codex_image_dispatch.py` 仍是内部出图适配，但主入口是 `butler.py`。
