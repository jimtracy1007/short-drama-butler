...
在**本集大纲确认后、正式剧本和分镜前**制作。短剧管家会把未登记的新角色、重要场景或道具整理成 `asset-production-plan.md`：其中包括类别、视觉说明、范围和出图提示词。你确认生产单后，可以用任何图片工具生成参考图；确认图片后才登记为本集素材、刷新交接包，随后继续剧本和分镜。

一次性背景或不重要的小道具可以在剧本完成后、对应关键帧出图前再制作；会反复出现的角色和场景应在正式分镜前锁定。
