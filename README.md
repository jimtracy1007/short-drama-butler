...
你只需要说故事、提供已有资料，并在关键节点回复“确认”或指出要改的地方。短剧管家自动读取项目记忆、整理文件和交接分镜，不需要你记编号、路径或 `pending` 目录。
...
flowchart TD
    ...
    P --> Q{"确认剧本与分镜？"}
    Q -->|要调整| P
    Q -->|确认| R["逐镜关键帧方案：首帧 / 尾帧 / 过程帧"]
    R --> S{"确认每镜帧数？"}
    S -->|要调整| R
    S -->|确认| T["关键帧执行单 + 按镜号整理图片"]
...
3. 确认正式剧本和分镜是否符合预期。
4. 确认每镜需要 1、2 还是 3 张关键帧。
5. 按 `keyframe-execution.md` 逐镜出图、生成视频并剪辑；本集完成后确认连续性记录。

### 你不必管的事

- 不必输入 `C01`、`S02` 等内部编号，也不必说“读取配置”或“创建交接包”。
- 不必手动维护素材索引、剧集状态和交接文件；短剧管家会自动更新它们。
- `keyframes/pending/` 只是“待生成图片”的内部预留文件名。出图时只看 `keyframe-execution.md`；图片生成后，短剧管家会按镜号和首 / 过程 / 尾帧归档。
...
多帧能让支持多参考帧的图生视频工具更好理解过程，但并非越多越好：每一张都必须是动作中的一个清晰阶段，不能只是近似重复图。你确认关键帧方案后，短剧管家才会开始生成图片；如果所用图生视频工具支持首尾帧，就同时上传该镜的首帧和尾帧，并使用执行单里的完整视频提示。

关键帧执行单不是一句“让角色动起来”的提示。它会逐镜继承 Storyboard Generator 的时长、景别、运镜、起点、过程、终点、台词、声音策略、音效、入点、出点 / 转场、素材参考和原分镜出图提示词；只额外增加 `KF<镜号>-start/middle/end.png` 的关键帧文件及对应出图提示词。因此同一集的故事、分镜、关键帧和图生视频不会各自变成另一套版本。
...
| `episodes/<集>/keyframe-execution.md` | 关键帧方案确认后 | 可直接执行的分镜镜像：保留时长、台词、声音、转场和原提示词，新增每张关键帧文件。 |
| `episodes/<集>/keyframe-execution-manifest.json` | Agent 或工具执行时 | 执行单的机器可读镜头、关键帧和状态数据。 |
...
**我能直接给一张图片吗？** 可以。说明它是谁或是什么场景，短剧管家会登记路径、范围和视觉准则。
