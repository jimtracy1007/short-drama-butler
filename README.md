...
flowchart TD
    ...
    T1 --> W
    T0 -->|齐全| W["按镜出静帧：当前镜有几张就派几个子代理；做完再开下一镜"]
    W --> W2{"确认静帧？"}
...
多帧能让支持多参考帧的图生视频工具更好理解过程，但并非越多越好：每一张都必须是动作中的一个清晰阶段，不能只是近似重复图。你确认关键帧方案后，短剧管家会建立 v2 执行单。夜戏若还缺对应场景母版，会先停下来补图。静帧由子代理按镜出图：当前镜有几张就同时派几个，做完审核后再开下一镜；精修可一次指出多张，每张一个子代理。主对话不自己画，只对照母版审核。图片工具由你选择，短剧管家不绑定或自动调用任何图片、视频或剪辑供应商。画漂了可以说要改哪里，指出几张就同时重做几张。

关键帧执行单不是一句“让角色动起来”的提示。它会逐镜继承 Storyboard Generator 的时长、景别、运镜、起点、过程、终点、台词、声音策略、音效、入点、出点 / 转场、素材参考和原分镜出图提示词，但静帧只画本帧画面：画风/时段/禁令留下，整段动作和运镜落幅不写进这一张。画漂了可以追加一句精修，不必整段重写。它的 Markdown 只显示当前确认版；配套的 `keyframe-execution-manifest.json` v2 记录每阶段实际输入、哈希、质检和完整版本链。因此同一集的故事、分镜、关键帧和图生视频不会各自变成另一套版本。
